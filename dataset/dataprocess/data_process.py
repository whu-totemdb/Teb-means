...
if __name__ == '__main__':
    ...
    config_file = "D:\BalanceKMeams\k-means\Code\Data\individually-fair-k-clustering-main\individually-fair-k-clustering-main\Dataprocess\example_config.ini"
    config = configparser.ConfigParser(converters={'list': read_list})
    config.read(config_file)

    # Create your own entry in `example_config.ini` and change this str to run
    # your own trial   athlete  bank census  creditcard diabetes recruitment  Spanish  student      yelp_academic_dataset_business census1990 hmda
    config_str = "census" if len(sys.argv) == 1 else sys.argv[1]
    ...
    # max_points = 10000
    num_clusters = 4
    for num_clusters in range(3, 10, 1):
        for  max_points in point_max:
            ...
            selected_columns1 = config[dataset].getlist("columns")
            df1 = df[[col for col in selected_columns1]]
        ...
        # df.to_csv('G:/F3KM/F3KM-MATLAB/'+dataset+'/'+dataset+'.csv', encoding="utf-8")
            ...
            df.to_csv ('D:\BalanceKMeams\k-means\Code\Data\individually-fair-k-clustering-main\individually-fair-k-clustering-main\yelp\output\\' + dataset+'_' + str(max_points) +'_' + str(num_clusters) + '.csv', encoding="utf-8", index=False)
