...
methods = {'kmeansPP', 'CDKM','BANCDKM_Ji',  };
                    ...
                    for rho_idx = 1:length(rho_set)
                        ...
                        for epsilon_idx = 1:length(epsilon_set)
                            ...
                            for ite_run = 1:num_runs
                                ...
                                switch method_name
                                    case 'kmeansPP'
                                        [Y_label, obj_max, elapsed_time] = kmeansPP(X, c);
                                        iter_num = 200;
                                        loss = NaN;
                                    case 'CDKM'
                                    ...
                                    case 'BANCDKM'
                                        [Y_label, ~, iter_num, obj_max, elapsed_time] = BANCDKM(X, label, c);
                                        loss = obj_max;
                                    case 'BANCDKM_Ji'
                                        [Y_label, ~, iter_num, obj_max, elapsed_time] = BANCDKM_Ji(X, label, c,  rho);
                                        loss = obj_max;
                                end
                            ...
                            figure('Visible', 'off');
                            plot(avg_loss);
                            file_name = sprintf('%s_%s_%.2f_obj.png', method_name, file_name_with_ext, rho);
